...
mongoose.connect("mongodb://127.0.0.1:27017/project").then(()=>console.log("Connected to mongodb"))
...
var registerSchema = new mongoose.Schema({name:String,phone:String,age:String,username:{type:String,unique:true},password:String,usertype:String},{versionKey: false})
...
app.delete("/api/delcatbyid/:uid",async(req,res)=>
{
    var result = await categorymodel.deleteOne({_id:req.params.uid});
...
app.delete("/api/delsubcatbyid/:uid",async(req,res)=>
{
    var result = await subcategorymodel.deleteOne({_id:req.params.uid});
...
app.put("/api/updateprods", upload.single('picture'),async (req, res)=>
{
    //var d = new Date();
    ...
    else
    {
      if(req.body.oldpicname!="noimage.jpg")
      {
        fs.unlink('public/upload/' + req.body.oldpicname, (err) => 
        {
          if (err)
          {
            console.log(err);
    ...
    var updateresult = await productModel.updateOne({ _id: req.body.prodid }, { $set: {catid:req.body.catid,subcatid:req.body.subcatid,prodname:req.body.pname,rate:req.body.rate,discount:req.body.discount,stock:req.body.stock,description:req.body.description,featured:req.body.featured,prodpic:picname,addedon:new Date}});
    if(updateresult.modifiedCount===1)
    {
        res.send({statuscode:1});
    }
...
app.put("/api/updatesubcategory",upload.single('picture'),async(req,res)=>
{
    if(!req.file)
    {
        ...
        if(req.body.oldpicname1="noimage.jpg")
        {
            fs.unlink('public/uploads/'+req.body.oldpicname,(err)=>
            {
                if(err)
...
app.delete("/api/deleteproduct/:uid",async(req,res)=>
{
    var result = await productModel.deleteOne({_id:req.params.uid});
...
app.post("/api/contactus",async (req, res)=> 
  {
      const mailOptions = {
      from: 'gadgetzone004@hotmail.com',
      to: 'gadgetzone004@hotmail.com',
  ...
  const transporter = nodemailer.createTransport({
    service : 'hotmail',
    auth : {
        user : 'gadgetzone004@hotmail.com',
        pass : 'S$n4Gj=HLPUVeF)'
    }
  })
