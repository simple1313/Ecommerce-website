...
var ManageCat=()=>
{
    ...
    var ondel= async(uid)=>
    {
        ...
        if(uchoice)
        {
            try 
            {
                const resp = await fetch(`http://localhost:9000/api/delcatbyid/${uid}`,
                {
                    method:"delete"
                })
                if(resp.ok)
			...
			<div className="login-form-grids">
				<form name="form1">
                   ...
                   {editmode?<img alt="catpic" src={`uploads/${catpic}`} height="75"/>:null}<br/>
					...
					<input type="button" name="btn" className="btn btn-primary"  onClick={onsubmit} value={editmode?"Update":"Add"}/>&nbsp;
