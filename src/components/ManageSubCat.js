...
var ManageSubCat=()=>
{
    ...
    var ondel= async(uid)=>
    {
        ...
        if(uchoice)
        {
            try 
            {
                const resp = await fetch(`http://localhost:9000/api/delsubcatbyid/${uid}`,
                {
                    method:"delete"
                })
                if(resp.ok)
		...
		<div className="container">
			...
			<div className="login-form-grids">
				<form name="form1">
					...
					<input type="button" name="btn" className="btn btn-primary"  onClick={onsubcatadd} value="Add Sub Category"/><br/>
					{msg}
				</form>
			</div>
            {
